    ...
    ui(new Ui::REV_HMI)
{
    ...
    dialog->show();
    connect(dialog, SIGNAL(updated(InputData)), this, SLOT(update_ui_test(InputData)));
#endif
...
void REV_HMI::enterPage()
{
 ...
 switch(tabIndex){
   case 1:{//log
        ...
        ui->homeBut->setDisabled(false);
        ui->battBut->setDisabled(false);
        ui->mapBut->setDisabled(false);
        ui->tripBut->setDisabled(false);
        ui->musicBut->setDisabled(false);
        ui->logBut->setDisabled(true);
    ...
    case 2: {//trip
        ...
        ui->battBut->setDisabled(false);
        ui->mapBut->setDisabled(false);
        ui->tripBut->setDisabled(true);
        ui->musicBut->setDisabled(false);
        ui->logBut->setDisabled(false);
...
void REV_HMI::update_ui_test(InputData inputData) {
    static InputData _inputData;
    static float accel50, drive_dist, secs;
    static int cnt_speed, max_speed;
    float avg_speed;

    //set new values
    if(inputData.setting_index != 0){
        if(inputData.setting_index%10 == 1){
            _inputData.speed = inputData.speed;
            _inputData.acceleration = inputData.acceleration;
            accel50 = 50/(float)inputData.acceleration;
        }

        if(inputData.setting_index/10%10 == 1){
            _inputData.drive_dist = inputData.drive_dist - drive_dist;
            qDebug()<<inputData.setting_index;
        }

        if(inputData.setting_index/100%10 == 1){
            _inputData.drive_time = inputData.drive_time;
        }

        if(inputData.setting_index/1000%10 == 1){
            _inputData.dtime = inputData.dtime;
            _inputData.drive_time = QTime(0,0);
            //cnt_time = QTime::fromString(inputData.dtime.toString("hh:mm:ss"), "hh:mm:ss");
        }

        if(inputData.setting_index/10000%10 == 1){
            _inputData.pcent = inputData.pcent;
            //cnt_time = QTime::fromString(inputData.dtime.toString("hh:mm:ss"), "hh:mm:ss");
        }

        InputData resetData;
        if(inputData.setting_index == -1){
            drive_dist = 0;
            secs =0;
            cnt_speed = 0;
            max_speed = 0;
    ...
    //calculations
        ...
        if(_inputData.acceleration >= 0) {
            if(cnt_speed < _inputData.speed){
             cnt_speed += _inputData.acceleration;
                cnt_speed = cnt_speed>_inputData.speed?inputData.speed:cnt_speed;
            }
        }
        else {
            if(cnt_speed > _inputData.speed){
        ...
        drive_dist += (float)(cnt_speed)/3.6/1000;
        _inputData.drive_dist = inputData.drive_dist + drive_dist;
        max_speed = cnt_speed>max_speed?cnt_speed:max_speed;

        _inputData.dtime = _inputData.dtime.addMSecs(1000);
        _inputData.drive_time = _inputData.drive_time.addMSecs(1000);
        secs++;
        avg_speed = drive_dist*1000/secs*3.6;

        QString s;
        //upgrade trip ui;
        ui->l_speed->setText(s.setNum(cnt_speed));
        ui->l_trip_Accel60->setText(s.setNum(accel50,'f',1));
        ui->l_trip_distance->setText(s.setNum(_inputData.drive_dist,'f',2));
        ui->l_trip_speedAvgElapsed->setText(s.setNum(avg_speed,'f',2));
        ui->l_trip_speedMax->setText(s.setNum((float)max_speed,'f',2));
        ui->l_trip_time->setText(_inputData.dtime.toString("hh:mm:ss"));
        ui->l_drive_time->setText(_inputData.drive_time.toString("hh:mm:ss"));

        //upgrade home ui;
        ui->val_tripDist->setText(s.setNum(_inputData.drive_dist,'f',2));
        ui->l_time->setText(_inputData.dtime.toString("hh:mm:ss"));
        ui->val_tripTime->setText(_inputData.drive_time.toString("hh:mm:ss"));

        //upgrade IVPlot
        ui->IVPlot->graph(1)->addData(secs,cnt_speed);
        ui->IVPlot->graph(1)->removeDataBefore(secs-60);
        ui->IVPlot->yAxis2->setRange(0,150);
        ui->IVPlot->xAxis->setRange(secs+0.25, 60, Qt::AlignRight);
        ui->IVPlot->replot();

        ui->progressBar_battery->setValue(_inputData.pcent);
//        ui->progressBar_battery->setStyleSheet("QProgressBar::chunk {background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop:0 rgb(240, 200, 0), stop:1 rgb(255, 230, 100);}");
//        ui->lowBatticon->hide();
    }
}
